    ...
    def __new__(cls, name, bases, attrs):
        ...
        for key, val in attrs.copy().items():
            ...
            if isinstance(val, Field):
                ...
                # pop(key)方法删除attrs中key对应的值，并返回这个值，将这个值与mappings的key对应
                mappings[key] = attrs.pop(key)
                logging.info('found mapping: %s ==> %s' % (key, val))
                # 查找并检验主键是否唯一，主键初始值为None，找到一个主键后会被设置为key，若if val.primary_key:
                # 再次为真，则会报错
                if val.primary_key:
                    if primary_key:
                        raise KeyError(
        ...
        attrs['__fields__'] = escaped_fields + \
            [primary_key]  # 将所有属性名都添加进 __fields__ 属性
        # -----------------------默认SQL语句--------------------------
        attrs['__select__'] = 'select * from `%s`' % (table)
        # `age`, `job`, `name`
        # ?, ?, ?
        attrs['__insert__'] = 'insert into `%s` (%s) values (%s)' % (
            table, ', '.join('`%s`' % f for f in mappings), ', '.join('?' * len(mappings)))
        attrs['__update__'] = 'update `%s` set %s where `%s` = ?' % (
            table, ', '.join('`%s` = ?' % f for f in escaped_fields), primary_key)
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def countRows(cls, selectField='*', where=None, args=None):
        ...
        sql = ['select count(%s) _num_ from `%s`' %
               (selectField, cls.__table__)]
        if where:
            sql.append('where %s' % (where))
